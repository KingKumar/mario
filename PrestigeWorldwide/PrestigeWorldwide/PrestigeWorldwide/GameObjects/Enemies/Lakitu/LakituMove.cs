...
{
    public class LakituMove : ILakituState
    {
        int frameDelay;
        int frameDelaySet=7;
        int currentFrame;
        ...
        public LakituMove(Level level)
        {
            this.level = level;
            this.texture = level.ContentLoad("Enemies/Lakitu/lakitu_movement");
            this.spriteBatch = level.spriteBatch;
            totalFrames = rows * columns / 2;
            frameDelay = frameDelaySet;
        }
        public Rectangle Update(Vector2 locaiton, int isMovingRight)
        {
            frameDelay--;
            if (frameDelay == 0)
            {
