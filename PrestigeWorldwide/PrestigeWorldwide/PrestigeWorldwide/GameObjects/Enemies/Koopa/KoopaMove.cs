...
{
    public class KoopaMove : IKoopaState
    {
        int frameDelay;
        int frameDelaySet=EnemyConstants.koopaMoveFrameDelay;
        int currentFrame;
        int rows = EnemyConstants.koopaMoveTextureRows;
        int columns = EnemyConstants.koopaMoveTextureColumns;
        Texture2D texture;
        int totalFrames;
        ...
        public KoopaMove(Level level)
        {
            this.level = level;
            this.texture = level.ContentLoad("Enemies/Koopa/koop_walking");
